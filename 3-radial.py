...
def get_radial_bounds(ring_count):
    return {"width": (1, 1 + ring_count * 2), "height": (1, 1 + ring_count * 2)}
...
def xy_coords(*, ring_number, spoke_number, ring_count, spoke_count):
    ...
    return [
        centre_x
        + ring_number * math.sin(math.pi - spoke_number / spoke_count * 2 * math.pi),
        centre_y
        + ring_number * math.cos(math.pi - spoke_number / spoke_count * 2 * math.pi),
    ]


def get_circular_arc_path_command(
    *, centre_x, centre_y, radius, start_angle, sweep_angle, angle_unit, sweep_flag=1
):
    """
...
def get_svg_radial_path_commands(radial_G, *, ring_count, spoke_count, arc_type):
    ...
    for (start, end) in radial_G.edges:
        ...
        if start == "C":
            end_x, end_y = xy_coords(
                **end, ring_count=ring_count, spoke_count=spoke_count
            )
            yield f"M {centre_x},{centre_y} L {end_x},{end_y}"
        elif end == "C":
            start_x, start_y = xy_coords(
                **start, ring_count=ring_count, spoke_count=spoke_count
            )
        ...
        # draw an arc between them.
            ...
            if arc_type == "straight":
                ...
                yield get_circular_arc_path_command(
                    ...
                    angle_unit="radians",
                    sweep_flag=spoke_number % 2
                    if arc_type == "alternating_curves"
                    else 0
                    if arc_type == "curved"
                    else 1,
                )
...
if __name__ == "__main__":
    ...
    G = generate_radial_graph(
        ring_count=ring_count,
        spoke_count=spoke_count,
        include_centre=random.choice([True, False]),
    )
    T = generate_spanning_tree_graph(G)

    bounds = get_radial_bounds(ring_count)
    path_commands = get_svg_radial_path_commands(
        T,
        ring_count=ring_count,
        spoke_count=spoke_count,
        arc_type=random.choice(
            ["curved", "alternating_curves", "inner_curve", "straight"]
        ),
    )
